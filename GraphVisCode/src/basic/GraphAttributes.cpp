...
void GraphAttributes::writeDungeon(const String &fileName) const
{
		ofstream os(fileName.cstr());
		writeDungeon(os);
...
string ogdfStringToSTDString(String ogdfS)
{
    const char* cStr = ogdfS.cstr();
    string normalString(cStr);

    return normalString;
}

void GraphAttributes::writeRoom(ostream &os, int x1, int x2, int y1, int y2, bool **layout, node v = NULL) const
{
    cout << x1 << " " << x2 << " " << y1 << " " << y2 << endl;

    atom atomToWrite;

    int rectMinX, rectMaxX, rectMinY, rectMaxY;
    ...
    {
        atomToWrite.allPreds.clear();
        atomToWrite.atomName = "roomID";
        atomToWrite.allPreds.push_back(intToString(rectMinX + 2));
        atomToWrite.allPreds.push_back(intToString(rectMinY + 2));
        atomToWrite.allPreds.push_back(ogdfStringToSTDString(labelNode(v)));
        os << atomToWrite.toString() << " ";

        atomToWrite.allPreds.clear();
        atomToWrite.atomName = "rectangle";
        atomToWrite.allPreds.push_back(intToString(rectMinX + 2));
        atomToWrite.allPreds.push_back(intToString(rectMinY + 2));
        atomToWrite.allPreds.push_back(intToString(rectMaxX + 2));
        atomToWrite.allPreds.push_back(intToString(rectMaxY + 2));
        atomToWrite.allPreds.push_back("room");
        os << atomToWrite.toString() << " ";
...
void writeCorridor(ostream &os, int x1, int x2, int y1, int y2, bool **layout)
{
    cout << x1 << " " << x2 << " " << y1 << " " << y2 << endl;
