...
typedef struct atom {
	char* atomType; //the name of the atom
	int numPreds; //the number of predicates
	char** preds; //array of predicates as c strings
} atom;

//crude linked list implementation 
typedef struct atomListEntry { 
	struct atomListEntry* next; //link to next entry in list
	atom* node; //the current entry in the list
} atomListEntry;

typedef struct pcgRoom {
	int roomID; //the id of the room
	int XUL, YUL, XBR, YBR; //the coordinates of the upper left and bottom right corners of the room
	
	//additional attributes as needed
} pcgRoom;

//crude linked list implementation 
typedef struct pcgRoomListEntry { 
	struct pcgRoomListEntry* next; //link to next entry in list
	pcgRoom* node; //the current entry in the list
} pcgRoomListEntry;

void removeAtom(atom* toRemove);
atomListEntry* removeEntry(atomListEntry* ent);
atomListEntry* addEntryToEnd(atomListEntry* tail, atom* atomToAdd);

void removeRoom(pcgRoom* toRemove);
pcgRoomListEntry* removeRoomEntry(pcgRoomListEntry* ent);
pcgRoomListEntry* addRoomEntryToEnd(pcgRoomListEntry* tail, pcgRoom* roomToAdd);
pcgRoom* findRoomID(pcgRoomListEntry* pcgRoomHead, int roomID);

void consumeLine(FILE* fp);
atom* parseAtomString(char atomString[100]);
 ...
 * Place the player at a random starting location.
 */
static void new_player_spot_in_room(struct cave *c, struct player *p, pcgRoom *roomToUse)
{
	int y, x;

	/* Try to find a good place to put the player */
	cave_find_in_range(c, &y, roomToUse->YUL, roomToUse->YBR, &x, roomToUse->XUL, roomToUse->XBR, cave_isstart);
...
static bool default_gen(struct cave *c, struct player *p) {
	int x, y;
	int levelStartRoom = -1;
	
	FILE *levelDef;	
	char curAtomString[100];
	atom* curAtom;
	pcgRoom* curRoom;
	atomListEntry *tileHead, *tileTail; //list of floor and door atoms
	atomListEntry *rectHead, *rectTail; //list of rectangle atoms
	atomListEntry *roomIDHead, *roomIDTail; //list of room ID atoms
	pcgRoomListEntry *pcgRoomHead, *pcgRoomTail;
	...
	levelDef = fopen("testLevel.pcg", "r");

	
	//consume the first 4 lines which contain no useful input
	consumeLine(levelDef);
	consumeLine(levelDef);
	consumeLine(levelDef);
	consumeLine(levelDef);		
	...
	while(strcmp(curAtomString, "SATISFIABLE"))
	{		
		curAtom = parseAtomString(curAtomString);
				
		if(!strcmp(curAtom->atomType,"levelLengthX"))
		{
			dunWidth=atoi(curAtom->preds[0]);
		}
		
		else if(!strcmp(curAtom->atomType,"levelLengthY"))
		{
			dunHeight=atoi(curAtom->preds[0]);
		}		
		
		else if(!strcmp(curAtom->atomType,"levelStartRoom"))
		{
			levelStartRoom=atoi(curAtom->preds[0]);
		}		
		
		
		else if(!strcmp(curAtom->atomType,"door") || (!strcmp(curAtom->atomType,"floor"))) //if there is a door or a floor fact		
		{
		
		
			if(tileHead == NULL) //if head is null start the list
			{
				tileHead = malloc(sizeof(atomListEntry));
				tileHead->next = NULL;
				tileHead->node = curAtom;
				
				tileTail = tileHead;
			}
			
			else //otherwise add atom to the end of tte list
			{
				tileTail = addEntryToEnd(tileTail,curAtom);
			}
		...
		else if(!strcmp(curAtom->atomType,"roomID")) //if there is a door or a floor fact		
		{
		
		
			if(roomIDHead == NULL) //if head is null start the list
			{
				roomIDHead = malloc(sizeof(atomListEntry));
				roomIDHead->next = NULL;
				roomIDHead->node = curAtom;
				
				roomIDTail = roomIDHead;
			}
			
			else //otherwise add atom to the end of tte list
			{
				roomIDTail = addEntryToEnd(roomIDTail,curAtom);
			}
						
		}
		
		else if(!strcmp(curAtom->atomType,"rectangle")) //if there is a door or a floor fact		
		{
		
		
			if(rectHead == NULL) //if head is null start the list
			{
				rectHead = malloc(sizeof(atomListEntry));
				rectHead->next = NULL;
				rectHead->node = curAtom;
			...
			{
				rectTail = addEntryToEnd(rectTail,curAtom);
			}
	...
	set_cave_dimensions(c, dunHeight, dunWidth);
	ROOM_LOG("height=%d  width=%d  nrooms=%d", c->height, c->width, 5);

	/* Initially fill with basic granite */
	fill_rectangle(c, 0, 0, dunHeight - 1, dunHeight - 1, FEAT_WALL_EXTRA);

	/* Generate permanent walls around the edge of the dungeon */
	draw_rectangle(c, 0, 0, dunHeight - 1, dunHeight - 1, FEAT_PERM_SOLID);

	//go through and place floors and doors
	while(tileHead != NULL)
	{
		curAtom = tileHead->node;
		
		if(!strcmp(curAtom->atomType,"door"))
		{
			terrainType = FEAT_DOOR_HEAD;
		}
		
		else
		{
			terrainType = FEAT_FLOOR;
		}
		
		//extract x and y
		x=atoi(curAtom->preds[0]);
		y=atoi(curAtom->preds[1]);
		
		fill_rectangle(c, y, x, y, x, terrainType);
		
		tileHead = removeEntry(tileHead);
	}
	
	//go through roomIDs as the first pass on creating rooms
	while(roomIDHead != NULL)
	{
		curAtom = roomIDHead->node;
		curRoom = parseRoomIDAtom(curAtom);
		
		if(pcgRoomHead == NULL) //if head is null start the list
		{
			pcgRoomHead = malloc(sizeof(pcgRoomListEntry));
			pcgRoomHead->next = NULL;
			pcgRoomHead->node = curRoom;
			
			pcgRoomTail = pcgRoomHead;
		}
		
		else //otherwise add atom to the end of tte list
		{
			pcgRoomTail = addRoomEntryToEnd(pcgRoomTail,curRoom);
		}

		roomIDHead = removeEntry(roomIDHead);
	}
	
	//go through rectangles to finish creating the rooms
	//go through roomIDs as the first pass on creating rooms
	while(rectHead != NULL)
	{
		curAtom = rectHead->node;
		
		finishRoom(pcgRoomHead,curAtom);
		
		rectHead = removeEntry(rectHead);
	}
	
	curRoom = findRoomID(pcgRoomHead, levelStartRoom);
	
	/* Determine the character location */
	new_player_spot_in_room(c, p,curRoom); //from line 560ish of this file. Should be trivial to create a new function that restricts the range of valid start tiles to the inside of a room. 

	/* Put some monsters in the dungeon */
	for (int i = 10; i > 0; i--)
		pick_and_place_distant_monster(c, loc(p->px, p->py), 0, TRUE, c->depth); //function in mon-make.c Want to actually use pick_and_place_monster which allows me to specify a location for the monster

	/* Put some objects in rooms */
	alloc_objects(c, SET_ROOM, TYP_OBJECT, Rand_normal(DUN_AMT_ROOM, 3), //really want alloc_object function at top of this file that allows me to allocate objects at particular locations
...
void removeAtom(atom* toRemove)
{
	if(toRemove->atomType != NULL)
	{
		free(toRemove->atomType);
	}
	
	if(toRemove->preds != NULL)
	{
		for(int i = 0; i < toRemove->numPreds; i++)
		{
			if(toRemove->preds[i] != NULL)
			{
				free(toRemove->preds[i]);
			}
		}
	}
...
//removes a entry from the linked list frees it's memory and returns pointer to next item in list
//assumes that entry is first entry in list right now
atomListEntry* removeEntry(atomListEntry* ent)
{
	atomListEntry* next = ent->next;
...
//given the end of a linked list creates a new entry in the list and returns a pointer to said list
atomListEntry* addEntryToEnd(atomListEntry* tail, atom* atomToAdd)
{
	atomListEntry* newTail = malloc(sizeof(atomListEntry));
	
	tail->next = newTail;
	newTail->next = NULL;
	newTail->node = atomToAdd;
...
pcgRoomListEntry* removeRoomEntry(pcgRoomListEntry* ent)
{
	pcgRoomListEntry* next = ent->next;
...
pcgRoomListEntry* addRoomEntryToEnd(pcgRoomListEntry* tail, pcgRoom* roomToAdd)
{
	pcgRoomListEntry* newTail = malloc(sizeof(pcgRoomListEntry));
	
	tail->next = newTail;
	newTail->next = NULL;
	newTail->node = roomToAdd;
...
pcgRoom* findRoomID(pcgRoomListEntry* pcgRoomHead, int roomID)
{
	pcgRoomListEntry* curEntry = pcgRoomHead;
	
	while(curEntry != NULL && curEntry->node->roomID != roomID) //while we have not reached the end of the list, or found a room with the same upper left corner as our rectangle
	{
		curEntry = curEntry->next;
	}
	
	assert(curEntry != NULL); //there should always be a matching room otherwise something went seriously wrong
	
	return curEntry -> node;
}

//consumes characters in a file until the end of the file or a new line character is reached
void consumeLine(FILE* fp)
{
...
//a version of strncpy that appends an end string character after the copied strings
void strncpyEndChar(char* dest, char* source, int numChars)
{
...
atom* parseAtomString(char atomString[100])
{
	...
	int numPreds = 0; //how many predicates are there
	
	//initalize values to be safe
	newAtom->atomType = malloc(sizeof(char) * 100);
	newAtom->numPreds = -1;
	newAtom->preds = NULL;
	
	//find where the name of the atom ends
	temp = strchr(atomString, '(');
	
	//copy the name
	strncpyEndChar(newAtom->atomType, atomString, (strlen(atomString) - strlen(temp)));
	
	//go past the atom name and ( in the string
	atomString = temp;
	atomString++;
	
	//count the number of predicates by seeing how many times you run 
	do
	{
		temp++; //go past the ( or , last found
		numPreds++;
		temp = strchr(temp, ','); //find the next ,
	} while(temp != NULL); //this is a do while not a while loop, since there will always be at least one predicate even if there is not a , to be found
	
	
	//finishing allocating memory
	newAtom->numPreds = numPreds;
	newAtom->preds = malloc(sizeof(char**) * numPreds);
	
	for(int i = 0; i < numPreds; i++)
	{
		newAtom->preds[i] = malloc(sizeof(char) * 100);
	}
	
	//finally store the predicates
	//for the first n-1 predicates we can use the same trick as with the atom name
	for(int i = 0; i < numPreds - 1; i++)
	{
		temp = strchr(atomString, ',');
	
		//copy the string
		strncpyEndChar(newAtom->preds[i], atomString, (strlen(atomString) - strlen(temp)));
		
		//go past the predicate and , in the string
		atomString = temp;
		atomString++;
	}
	
	//the last predicate takes up all but 1 character (the ')' at the end) so copy all but the last character
	strncpyEndChar(newAtom->preds[numPreds-1], atomString, (strlen(atomString) - 1));
	
	return newAtom;
}

//takes a roomID atom and creates the begining of a room struct for it
pcgRoom* parseRoomIDAtom(atom* roomID)
{
	pcgRoom* newRoom = malloc(sizeof(pcgRoom));
	
	newRoom->XUL = atoi(roomID->preds[0]);
	newRoom->YUL = atoi(roomID->preds[1]);
	newRoom->roomID = atoi(roomID->preds[2]);
	
	return newRoom;
}

//takes a linked list of rooms, and a rectangle atom, and finds the room the rectangle is part of and uses the rectangle to fill in the XBR,YBR values
void finishRoom(pcgRoomListEntry* head, atom* rectangle)
{
	int XUL = atoi(rectangle->preds[0]);
	int YUL = atoi(rectangle->preds[1]);
	
	pcgRoomListEntry* curEntry = head;
	
	while(curEntry != NULL && (curEntry->node->XUL != XUL || curEntry->node->YUL != YUL)) //while we have not reached the end of the list, or found a room with the same upper left corner as our rectangle
	{
		curEntry = curEntry->next;
	}
	...
	//read in and store the bottom right corner
